    ...
    <>
      <nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          shouldBeWhite
            ? 'bg-[#FEFCF8]/85 backdrop-blur-3xl shadow-sm border-b border-[#EDE4D8]/40'
              ...
              {navCategories
                ...
                .map((cat) => (
                  <Link
                    ...
                    href={`/${cat.slug}`}
                    prefetch={false}
                    className={`px-4 py-2 text-sm font-body transition-colors duration-300 relative group ${shouldBeWhite ? 'text-[#1E1410] hover:text-black font-semibold' : 'text-white hover:text-white/80 font-semibold'
                      }`}
                  >
                    {cat.name}
                  ...
                  <div className="flex items-center gap-3">
                    {config?.social_links?.instagram && (
                      <a href={config.social_links.instagram} target="_blank" rel="noopener noreferrer" className="transition-transform duration-150 hover:scale-110">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={shouldBeWhite ? "#1E1410" : "#FFFFFF"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {whatsappUrl && (
                      <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="transition-transform duration-150 hover:scale-110">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill={shouldBeWhite ? "#1E1410" : "#FFFFFF"}>
            ...
            {navCategories
              ...
              .map((cat) => (
                <Link
                  ...
                  className="block px-4 py-3 text-sm font-body font-medium text-[#1E1410] hover:bg-black/5 rounded-sm transition-all duration-200"
                >
                  {cat.name}
                </Link>
              ))}
      ...
      <div className="fixed left-0 bottom-24 z-40 hidden max-md:flex flex-col items-center gap-2 bg-black/30 backdrop-blur-md py-3 pl-1 pr-2 rounded-r-2xl shadow-[4px_4px_16px_rgba(0,0,0,0.25)] border-y border-r border-white/10">
        {config?.social_links?.facebook && (
          <a
            ...
            className="w-7 h-7 rounded-full bg-white text-[#1E1410] flex items-center justify-center transition-all duration-200 active:scale-90"
          >
