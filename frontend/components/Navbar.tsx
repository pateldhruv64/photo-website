              ...
              {navCategories
                ...
                .map((cat) => (
                  <Link
                    key={cat._id}
                    href={`/${cat.slug}`}
                    className={`px-4 py-2 text-sm font-body transition-colors duration-300 relative group ${scrolled ? 'text-[#1A1A1A] hover:text-black font-semibold' : 'text-white hover:text-white/80 font-semibold'
                      }`}
                  >
                    {cat.name}
                  ...
                  <div className="flex items-center gap-3">
                    {config?.social_links?.instagram && (
                      <a href={config.social_links.instagram} target="_blank" rel="noopener noreferrer" className="transition-transform duration-150 hover:scale-110">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={scrolled ? "#1A1A1A" : "#FFFFFF"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
